...
<body>       
    ...
    <main>
        <section class="registerPage">
            <div class="formBox">
                <h1>Edit <span style="color: rgb(154, 228, 43);">Distro</span></h1>
                <form action="/{{ $distro[0]->id }}/edit/put" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    @error('name')
                    <p style="color: red; font-size: 13px;">{{ $message }}</p>
                    @enderror
                    <input type="text" name="name" placeholder="Distro's name" value="{{ $distro[0]->name }}">

                    @error('company')
                    <p style="color: red; font-size: 13px;">{{ $message }}</p>
                    @enderror
                    <input type="text" name="company" placeholder="Company's name" value="{{ $distro[0]->company }}">

                    @error('short_description')
                    <p style="color: red; font-size: 13px;">{{ $message }}</p>
                    @enderror
                    <input type="text" name="short_description" placeholder="Add a short description of your distro" maxlength="80" value="{{ $distro[0]->short_description }}">
                    
                    @error('website')
                    <p style="color: red; font-size: 13px;">{{ $message }}</p>
                    @enderror
                    <input type="text" name="website" placeholder="URL of your distro's website" value="{{ $distro[0]->website }}">

                    @error('description')
                    <p style="color: red; font-size: 13px;">{{ $message }}</p>
                    @enderror
                    <textarea name="description" cols="30" rows="10">{{ $distro[0]->description }}</textarea>

                    @if($distro[0]->image == null)
                    <img src="{{ asset('/resources/images/logo.ico') }}" alt="Standart" style="width: 100px; height: 100px; object-fit: cover;">
                    @else   
                    <img src="{{ asset('/storage/'.$distro[0]->image) }}" alt="Standart" style="width: 100px; height: 100px; object-fit: cover;">
                    @endif
                    ...
                    <p style="color: red; font-size: 13px;">{{ $message }}</p>
                    @enderror
                    <label for="image"><p style="font-size: 14px;">Change or add an image to your distro (not required)</p></label>
